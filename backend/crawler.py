            ...
            for entry in feed.entries[:max_results // len(ARXIV_CATEGORIES)]:
                ...
                try:
                    ...
                    paper_data = {
                        ...
                        "updated_date": paper.updated if hasattr(paper, "updated") else paper.published,
                        "citations": 0,  # 기본값, 실제로는 외부 API 필요
                        "trending_score": 0.0,
                        "recency_score": calculate_recency_score(paper.published) if paper.published else 0.0,
                    }
...
def calculate_recency_score(published_date: datetime) -> float:
    ...
    now = datetime.now(published_date.tzinfo) if published_date.tzinfo else datetime.utcnow()
