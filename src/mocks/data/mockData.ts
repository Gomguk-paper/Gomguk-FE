// This file contains mock data for MSW handlers
// Moved from src/models/papers.ts and src/data/authors.ts

import type { Paper, Summary, Report } from '@/models';
import { type Author, authors } from '@/data/authors';

const baseMockPapers: Paper[] = [
    {
        id: "p1",
...
// Generate more mock papers for infinite scroll testing
const generatedPapers: Paper[] = Array.from({ length: 50 }).map((_, i) => {
    const basePaper = baseMockPapers[i % baseMockPapers.length];
    return {
        ...basePaper,
        id: `generated-p${i + 13}`,
        title: `${basePaper.title} (Vol. ${i + 1})`,
        metrics: {
            ...basePaper.metrics,
...
export const mockPapers: Paper[] = [...baseMockPapers, ...generatedPapers];
...
// Re-export authors from source of truth
export const mockAuthors: Author[] = [...authors];
...
export const mockTagDescriptions: Record<string, string> = {
    ...
    "Vision": "컴퓨터 비전 및 이미지 처리",
    "RL": "강화학습 (Reinforcement Learning)",
    "Diffusion": "확산 모델을 사용한 생성 모델",
    "LLM": "대규모 언어 모델 (Large Language Model)",
    "Generative": "생성형 AI 모델",
    "Attention": "어텐션 메커니즘",
    "CNN": "합성곱 신경망 (Convolutional Neural Network)",
    "RLHF": "인간 피드백을 활용한 강화학습",
    "Multimodal": "다중 모달 (텍스트, 이미지 등 여러 형태의 데이터 처리)",
