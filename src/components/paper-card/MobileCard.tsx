...
export function MobileCard({ paper, onOpenSummary }: PaperCardProps) {
    const {
        showLoginModal,
        setShowLoginModal,
        hideToastPhase,
    ...
    return (
        <>
            <article
                className="bg-card rounded-lg border shadow-card transition-all cursor-pointer overflow-hidden flex flex-col"
                onClick={handleCardClick}
            >
                {/* Image Section (2:1 aspect ratio) */}
                {imageUrl && (
                    <div className="w-full flex-shrink-0">
                        <div className="bg-gray-50/50 border-b relative aspect-[2/1] w-full overflow-hidden">
                            <img
                                src={imageUrl}
                ...
                <div className="flex-1 flex flex-col p-4 pb-0">
                    ...
                    {/* Header: Title & Why (No actions here if image exists) */}
                    ...
                    {/* Actions (Compact py-1) */}
                    <div className="flex flex-wrap items-center gap-2 py-3 border-t mt-4">
                            ...
                            <Tooltip>
                                <TooltipTrigger asChild>
                                    <Button
                                        variant="ghost"
                                        size="icon"
                                        className={cn("h-11 w-11 p-2 text-muted-foreground hover:text-liked hover:bg-red-50", isLiked && "text-liked hover:text-liked bg-red-50")}
                                        onClick={(e) => {
                                            e.stopPropagation();
                            ...
                            <Tooltip>
                                <TooltipTrigger asChild>
                                    <Button
                                        variant="ghost"
                                        size="icon"
                                        className={cn("h-11 w-11 p-2 text-muted-foreground hover:text-saved hover:bg-blue-50", isSaved && "text-saved hover:text-saved bg-blue-50")}
                                        onClick={(e) => {
                                            e.stopPropagation();
